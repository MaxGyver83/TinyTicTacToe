...
init_game()
{
	...
	char filename[6];
	for (int i = 0; i < 10; i++) {
		snprintf(filename, sizeof(filename), "%d.pgm", i);
	...
	t_levels[1] = load_texture("easy.pgm");
	t_levels[2] = load_texture("medium.pgm");
	t_levels[3] = load_texture("hard.pgm");
	t_levels[4] = load_texture("very_hard.pgm");
